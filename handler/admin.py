...
from database import (
    is_admin_by_id, all_books, save_book, get_orders, get_book_by_id,
    get_user_by_id, get_all_users, get_all_orders, get_all_admin, get_all_books
)
from buttons import (
    main_button_admin, menu_kb, menu_admin_button, order_button, order_changed,
    order_finish, ADMIN_MAIN_TEXT, MAIN_TEXT, ADMIN_MENU, back_button, ORDERS_TEXT,
    convert_books_to_html
)
...
async def menu_admin_all(message: Message):
    chat_id = message.from_user.id
    if is_admin_by_id(chat_id):
        books = all_books()
        ...
        pdfkit.from_string(
            books_html, "books.pdf", options={"enable-local-file-access": ""}, configuration=config
        )
...
async def get_new_orders(message: Message):
    ...
    for order in orders:
        order_id, book_id, user_id, price, quantity, _, create_at = order
        # None tekshiruvi
        book_name = "Noma'lum kitob"
        user_name = "Noma'lum foydalanuvchi"
        ...
        await message.answer(text, reply_markup=order_changed(order_id))


@admin_router.callback_query(F.data.startswith("in_progress"))
async def change_order_in_progress(call: CallbackQuery):
    order_id = call.data.split("_")[-1]
    change_status("in_progress", order_id)
...
async def show_dashboard(message: Message):
    ...
    summa = sum(int(order[3]) * int(order[4]) for order in get_all_orders())
    text = f"Users: {users}\nBooks: {books}\nAdmins: {admins}\nTotal: {summa}"
